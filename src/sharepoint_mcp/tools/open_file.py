...
"""sp_drive_file_checkout — checkout a SharePoint file and download its content.
...
registers the entry in `CheckoutRegistry` so `sp_drive_file_checkin` can find the
ETag for stale-write detection and `sp_drive_file_checkout_discard` knows what to discard.
...
def open_file(
    ...
    *,
    ...
    1. Server-side checkout lock acquired (others see "checked out by
       <test-user>" until `sp_drive_file_checkout_discard` or `sp_drive_file_checkin` is called).

    Side effects (local):

    2. File content written to
       `<base_dir>/<profile>/working/<item-id>/<filename>`.
    3. `CheckoutRegistry` entry created so subsequent `sp_drive_file_checkin` knows
    ...
    if not url or not url.strip():
        raise ValueError("sp_drive_file_checkout requires a non-empty url")

    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(f"sp_drive_file_checkout needs a file URL, got {url!r}")
