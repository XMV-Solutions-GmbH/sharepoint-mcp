...
"""sp_drive_file_read — download a SharePoint file's content to a local temp file.

Read-only. Does NOT acquire a checkout / lock — that's `sp_drive_file_checkout`'s job.
...
def read_file(
    ...
    *,
    ...
    if not url or not url.strip():
        raise ValueError("sp_drive_file_read requires a non-empty url")

    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(
            "sp_drive_file_read needs a file URL, got a site/folder URL "
            f"with no item path: {url!r}",
        )
