...
"""sp_drive_file_history — list version history of a SharePoint file.
...
sp_drive_file_checkin call is real (visible in the SharePoint web UI's version
...
def history(
    ...
    *,
    ...
    - `id` — version identifier (e.g. "3.0", "3.1") — pass to
      `sp_drive_file_version_get` to fetch that version's content.
    ...
    if not url or not url.strip():
        raise ValueError("sp_drive_file_history requires a non-empty url")
    if limit <= 0:
        raise ValueError(f"limit must be positive, got {limit}")

    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(f"sp_drive_file_history needs a file URL, got {url!r}")
