...
- `sp_share_link_list(url)` — list existing sharing links on an item
- `sp_share_link_create(url, type, scope, expires=None, password=None)` —
  create a sharing link, returns the share URL
- `sp_share_link_revoke(url, link_id)` — delete a sharing-link permission
...
- All three tools are gated by `SP_ALLOW_WRITES` at the server layer
  (yes including `sp_share_link_list` — it doesn't mutate but it's
  ...
  Actually no: `sp_share_link_list` is read-only. It stays in the read
...
def share_list(
    ...
    *,
    ...
    This is equivalent to `sp_share_permission_list(url)` filtered to entries
    whose grantee is a sharing link. Returned shape matches what
    `sp_share_link_create` produces, so callers can correlate.

    Returns an empty list when no sharing links exist on the item.
    """
    if not url or not url.strip():
        raise ValueError("sp_share_link_list requires a non-empty url")
    _, _, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(
            f"sp_share_link_list requires a file/folder URL; got a site URL: {url!r}",
        )
...
def share_create(
    ...
    *,
    ...
    """
    if not url or not url.strip():
        raise ValueError("sp_share_link_create requires a non-empty url")
    if type not in VALID_LINK_TYPES:
        raise ValueError(
            f"sp_share_link_create type must be one of {sorted(VALID_LINK_TYPES)}; got {type!r}",
        )
    if scope not in VALID_LINK_SCOPES:
        raise ValueError(
            f"sp_share_link_create scope must be one of {sorted(VALID_LINK_SCOPES)}; got {scope!r}",
        )
    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(
            f"sp_share_link_create requires a file/folder URL; got a site URL: {url!r}",
        )
...
def share_revoke(
    ...
    *,
    ...
    `link_id` is the permission id returned by `sp_share_link_create` or
    `sp_share_link_list`. After this call the share URL stops working.
    """
    if not url or not url.strip():
        raise ValueError("sp_share_link_revoke requires a non-empty url")
    if not link_id or not str(link_id).strip():
        raise ValueError("sp_share_link_revoke requires a non-empty link_id")
    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(
            f"sp_share_link_revoke requires a file/folder URL; got a site URL: {url!r}",
        )
...
def _normalise_existing_link(perm: dict[str, Any]) -> dict[str, Any]:
    """Re-shape a sp_share_permission_list entry into the same shape as create.

    sp_share_link_list calls sp_share_permission_list and filters to link grantees;
