...
"""sp_drive_file_version_get — download a specific historical version of a file.

Read-only. Wraps `GET /drives/{id}/items/{id}/versions/{version-id}/content`.

Same overall shape as `sp_drive_file_read` (writes the bytes to a temp file with
the original extension preserved, returns the path) but addressed by
version-id instead of "current". Use `sp_drive_file_history` first to discover
...
def get_version(
    ...
    *,
    ...
    `version_id` is the id from `sp_drive_file_history`'s response (e.g. "3.0").
    ...
    if not url or not url.strip():
        raise ValueError("sp_drive_file_version_get requires a non-empty url")
    if not version_id or not version_id.strip():
        raise ValueError("sp_drive_file_version_get requires a non-empty version_id")

    hostname, site_path, item_path = parse_sharepoint_url(url)
    if not item_path:
        raise ValueError(f"sp_drive_file_version_get needs a file URL, got {url!r}")
