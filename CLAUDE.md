...
- **Harness token renewal.** Monthly recurring chore: Microsoft refresh tokens rotate every ~60-90 days, so the `SHAREPOINT_HARNESS_TOKEN_JSON` repo secret has to be refreshed before CI's harness job starts failing. Run `./scripts/renew-harness-token.sh` — single-command flow with Device Code login + `gh secret set`. Document is in [docs/testconcept.md § Renewing the harness token](docs/testconcept.md).
